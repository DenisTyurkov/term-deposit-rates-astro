...
- [x] **Phase 3 — verification** (2026-07-25)
  ...
  **Change (Denis, 2026-07-25):** ActiveCampaign list/tag are hardcoded in `SITE_CONFIGS` (`listId: '1'` = "Wise_Content_leads", `tagId: '28'` = "new_subscribers_tdrates") instead of env vars — they're not secrets and don't vary per environment. `TERMDEPOSIT_AC_LIST_ID`/`TERMDEPOSIT_AC_TAG_ID` no longer exist. nz-leads commit amended to `afe4287`. Context discovered along the way: nothing in nz-leads pushes broadband leads to AC (the only historical caller was the Glimp import, deleted Feb 2026); the daily "via API" contacts in Wise_Content_leads come from an automation outside the repo (likely LeavyStudio-operated; `/api/leads/json` serves last-7-days leads unauthenticated and is the likely feed). Since TD leads share list 1 and are segmented by tag 28, any external sweep is harmless. Follow-ups for Denis: ask LeavyStudio what feeds AC; consider auth on `/api/leads/json`.

- [x] **LIVE — verified end-to-end in production (2026-07-25)**
  - nz-leads `afe4287` pushed to `master` + `prod`; new build live after ~6.5 min; Denis had pre-set `TERMDEPOSIT_CAMPAIGN_ID` + both TD origins in Vercel env, picked up by the same build.
